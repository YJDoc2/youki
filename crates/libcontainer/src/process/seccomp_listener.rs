...
use std::{io::IoSlice, path::Path, os::fd::AsRawFd};
...
fn sync_seccomp_send_msg(listener_path: &Path, msg: &[u8], fd: i32) -> Result<()> {
    ...
    socket::sendmsg::<UnixAddr>(socket.as_raw_fd(), &iov, &[cmsgs], socket::MsgFlags::empty(), None).map_err(
        |err| {
    ...
    let _ = unistd::close(socket.as_raw_fd());
